...
#  try: 
    api = urllib.urlopen('http://finance.yahoo.com/webservice/v1/symbols/'+ticker+'/quote?format=json')
    response = api.read()
    response_dictionary = json.loads(response)
    stock_name = response_dictionary['list']['resources'][0]['resource']['fields']['name'].replace("Common Stock", '').replace(' Inc','').replace(',','').replace('.','')
